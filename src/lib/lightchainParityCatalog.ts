...
const lightchainLauncherDescriptionOverrides: Record<string, string> = {
  'marketing-workspace': '販促コンテンツのオールインワン制作 バナー、キャッチコピー、商品ページまで。AIがあらゆるマーケティング素材を自動生成します。',
  'virtual-fitting': 'バーチャル撮影アシスタント 場所を選ばず、高品質なモデル着用画像を生成。スタジオ撮影の手間とコストを大幅に削減。',
  'wear-design-lab': 'ディテールカスタマイズ 理想のスタイルを細部まで。ピンポイントな修正で独自の1着を構築',
  'video-workstation': 'AIプロモーション動画生成 製品紹介や着せ替えエフェクトなど、動的なマーケティング動画をクイックに生成。',
  'model-library': 'カスタムモデル生成 顔立ち、ポーズ、体格、肌の色まで。ターゲットに合わせた理想のモデルを自由自在に設定。',
  'fashion-studio': 'スマート・スタイリング 衣服、シーン、小物を自在に組み合わせ。AIが自然に融合させ、独自のコーディネート案を形にします。',
  'design-agent': 'トレンド企画アシスタント 最新トレンドを分析し、アイデアを具体的な企画書やデザイン案へ。企画の「勝ち筋」を瞬時に可視化。',
  'heavychain-lab': '開発中の機能をお試しいただけます。',
  'inspiration-design': 'AIデザイン・ジェネレーター 生地やインスピレーションから独創的なデザイン案を無限に創出。ゼロからのクリエイティブを支援。',
  'fabric-simulation': 'サンプルレス・プレビュー 柄、生地、プリントの着用感を即座に確認。サンプル制作（トワル）なしで仕上がりを可視化。',
  'lineart-to-real': '線画・実写の双方向変換 デザイン線画と製品イメージ（実写風）を瞬時に相互変換。イメージの具体化と共有を加速させます。',
  'change-color': 'カラーカスタマイズ 全体またはパーツ単位での配色変更。',
  'flat-vector': 'デザイン線画のベクター化 線画を瞬時にベクターデータへ。',
  'custom-style': 'ブランドスタイルの統一 独自のスタイルを自由に設定。すべてのアイテムで一貫したブランドの世界観を維持します。',
  'remove-background': '人物・歪み自動補正 顔、手足、衣装の歪みをAIが自動リサーチ。細部まで自然に整えます。',
  'graphic-design': 'AIパターン生成 オリジナルの柄やプリントをAIが自動生成。多彩なデザインニーズにこれ一つで応えます。',
  'pattern-vector-pro': 'ベクター変換(PRO) あらゆる柄をワンクリックで編集可能なベクターデータへ。製造現場にそのまま入稿できる品質を実現。',
  'design-arrange': 'デザイン修正 柄の差し替え・微調整をスマートに。細部まで精緻にカスタマイズし、加工の質感までリアルに再現。',
...
export const lightchainParityGoals: LightchainParityGoal[] = [
  {
    ...
    owningSurface: 'GeneratePage / MaterialWorkbench / /model',
  ...
  },
  {
    matrixId: 'M05',
    title: '動画ワークステーション',
    lightBehavior: 'project dashboard → guide/dropzone detail → storyboard workspace',
    heavyCurrentBehavior: 'Canonical dashboard/detail now match the Light entry and initial dropzone; provider admission remains fail-closed',
    heavyTarget: 'retain the canonical dashboard/detail flow and connect storyboard generation only after an approved provider receipt path exists',
    priority: 'P1',
    rationale: 'entry and detail parity is required for the full video workspace while live provider completion remains a separate gate',
    owningSurface: 'GeneratePage / /flow/GenerateShortVideo',
    acceptanceEvidence: 'source/Heavy production readback plus local desktop/mobile dashboard and detail workflow evidence; provider receipt and save/reuse remain open',
    status: 'in_progress',
  },
  {
    matrixId: 'M06',
