...
Heavy Chain には、認証、ブランド設定、クレジット、ジョブ、履歴、ギャラリー、キャンバス、生成系 Edge Functions、Supabase storage/DB まわりの release 証跡があります。production app は 2026-06-22 時点で commit `977ddb60e88de032c902c10df7622c401e6d77e1` の push、Edge Functions deploy、production readback、public auth QA、release doctor all OK まで完了しています。ただし `workspace-source-generation-local-20260622` はこの earlier production proof の対象外です。最新 Goal は、既存ワークスペース体験を実生成と server-side workflow/readback に接続する local 実装までは完了しており、production closeout は明示承認後の Web bundle / Edge Function deploy と live DB/Storage readback が必要です。
...
推奨順は、明示承認後に `/patterns`、`/studio`、`/video`、`/lab` の順で production live generation と server-side readback を closeout することです。Marketing と Fitting/Models の production deploy/readback は完了していますが、最新 `workspace-source-generation-local-20260622` は local proof 止まりです。次は未接続ワークスペースの生成ジョブ、Storage/DB readback、進捗、失敗復帰、途中編集、履歴復帰を本番サーバー状態で証跡化します。
...
現在の Goal の次スライスは「未接続ワークスペースの production live generation と server-side workflow/readback closeout」です。完了条件は、認証済みユーザーが `/studio` `/patterns` `/video` `/lab` から作った実生成ジョブを Supabase DB/Storage の履歴、ギャラリー、キャンバスで読み戻せ、再ログインまたは別ブラウザ相当でも確認でき、進捗、成功、停滞/失敗、再試行、編集画面への handoff を Playwright で証跡化できることです。`output/playwright/workspace-source-generation-local-20260622/verification-summary.json` は local proof であり、production DB/Storage readback の代替にはしません。
