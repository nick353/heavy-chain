...
The `服をタップしてAIマスク` flow runs the revision-pinned EfficientSAM-Ti point-prompt model with ONNX Runtime's single-threaded WASM backend inside a Vite-emitted module worker. The Blob-backed ORT proxy worker is deliberately disabled inside that dedicated worker because a proxy bootstrap failure on the deployed surface would silently demote the recommended tap flow to the rectangular fallback; model loading and inference remain off the UI thread. It passes exactly the operator's point, rejects masks that miss the point, dominate the frame, or fall outside the bounded area policy, and shows the accepted pixels in blue before any downstream change. The operator must press `決定`; the reviewed mask is exported as source alpha and preserved exactly by the cutout pipeline. A low-confidence neighbourhood remains an explicitly labelled rectangular fallback, but confirmation routes it through the established range/silueta path rather than `u2net_cloth_seg`. EfficientSAM load or inference failure also keeps range and manual keep/remove correction available. The older semantic cloth parser remains a legacy validator/fallback dependency and is not prewarmed or selected by the recommended tap lane. The tap proposal is not proof of semantic garment-part recognition, and the UI must not label a fallback as semantic success.
