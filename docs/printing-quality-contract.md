...
The `服をタップしてAIマスク` flow records the operator's tap intent and forwards the bounded crop to the cutout pipeline only after the operator reviews the blue pixel proposal and presses `このマスクで確定`. A low-confidence neighbourhood remains an explicitly labelled rectangular fallback. The editor offers the reference flow's three categorical targets: `トップス`, `ボトムス`, and `全身`, mapped to the cloth model's upper, lower, and full-body masks at indices 0, 1, and 2. The chosen target is part of the current generation identity and completed cloth-result provenance; a semantic success label requires the requested and completed targets to match. A tap selects `u2net_cloth_seg` only when the build explicitly provides `VITE_REMBG_CLOTH_SEG_MODEL_URL`; the Zeabur `build:deploy` command supplies the pinned same-origin `/models/u2net_cloth_seg.onnx`, while ordinary builds without that variable remain on the existing silueta/manual fallback. Automatic selection and rectangle/range selection never switch models implicitly. The confirmed blue proposal is exported as source alpha. When the cloth model is configured, neither that transparency nor the uniform-background fast path may silently bypass the requested model: semantic inference runs first and its output is intersected with the confirmed source alpha so it can remove pixels but cannot expand the printable mask. Model load/inference failure, a missing/mismatched class mask, or a non-cloth result engine preserves the confirmed transparent input and still leaves the existing candidate picker and manual keep/remove editor available. The tap proposal is not proof of garment-part recognition, and the UI must not label a fallback as semantic success.

The cloth model has a dedicated 90-second `predict()` budget inside a cloth-only 105-second page-level cutout budget. In the installed wrapper, the first `predict()` includes the approximately 176 MB model fetch, session initialization, normalization, inference, and mask conversion; `newSession()` alone does not perform that work. The smaller default model keeps the existing 30-second rembg operation and 75-second page budgets. Repeated local compatibility diagnostics measured raw `session.run()` from about 25.4 to 48.4 seconds on one model-worn screenshot; this does not prove browser `predict()` end-to-end timing. ONNX Web cannot be cancelled through the installed wrapper, so a timed-out cloth prediction remains marked in-flight until it settles and another cloth inference is refused instead of overlapping. A timeout or busy result still returns to the confirmed tap mask and manual editor.
...
When a `manual-ready` printable surface is explicitly enabled, the app may add a third result labelled `布面メッシュ追従（試験）`. It uses bounded 2D luminance gradients, premultiplied-alpha resampling, an adaptive printable-panel row mesh, and restrained fold shading. The adaptive mesh tolerates short occlusion/multi-run rows by selecting the dominant central run and interpolating only small gaps; large or unstable profiles remain out-of-domain. The regular fabric result also accepts optional stage dimensions so it can add a bounded local fold-contrast term without changing geometry or alpha. It is not automatic surface recognition, a 3D simulation, or a realism guarantee.
...
The 2D experimental conformer now uses a bounded ROI wrapper at 1440×1800 instead of a blanket skip. It scans the full clip with the same deadline, keeps the legacy direct path unchanged below the one-million-pixel budget, and returns a typed ROI-too-large failure if the crop itself would exceed the safety budget. An optional encoded occluder plane can travel through the snapshot only when explicitly supplied; it is hash/dimension/RGB/capacity validated and forwarded to the experimental conformer, while omitted occluders preserve the existing snapshot and output path. Exact and fabric results remain downloadable at the selected dimensions. These PNGs are higher-resolution mockups, not manufacturing-ready DPI, CMYK, bleed, or ICC output.
...
The same conformer retains the legacy narrow panel-profile warp when the clip alpha profile is stable enough to measure from `alpha >= 64` rows. Every row between the first and last active row must contain exactly one contiguous run; any empty row inside the span, multi-run row, or other profile break disables that legacy warp and preserves the disabled export byte-for-byte. The explicitly labelled experimental surface lane opts into an adaptive mesh mode: it selects the dominant central run, interpolates only short gaps, applies a bounded vertical shear in addition to row-width horizontal warp, and keeps the existing gradient displacement capped independently. Both modes remain fail-closed for unstable or frame-touching surfaces.
