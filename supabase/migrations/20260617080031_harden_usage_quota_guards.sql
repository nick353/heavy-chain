-- Harden usage reservations with stale release and short-window quota guards.

CREATE INDEX IF NOT EXISTS idx_usage_events_status_reserved_at
  ON public.usage_events(status, reserved_at)
  WHERE status = 'reserved';

CREATE INDEX IF NOT EXISTS idx_usage_events_brand_recent_reserved_succeeded
  ON public.usage_events(brand_id, reserved_at DESC)
  WHERE status IN ('reserved', 'succeeded');

CREATE INDEX IF NOT EXISTS idx_usage_events_user_recent_reserved_succeeded
  ON public.usage_events(user_id, reserved_at DESC)
  WHERE status IN ('reserved', 'succeeded');
