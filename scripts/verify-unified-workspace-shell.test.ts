...
test('first complete apparel flows use the unified workspace shell', () => {
  ...
  assert.match(shell, /data-testid="heavy-unified-workspace-shell"/);
  assert.match(shell, /data-testid="heavy-unified-feature-library"/);
  assert.match(shell, /data-testid="heavy-unified-context-rail"/);
  assert.match(shell, /data-flow-state=\{flowState\}/);
  assert.match(shell, /data-testid="heavy-unified-flow-state"/);
  assert.match(shell, /unifiedWorkspaceFlowLabels\[flowState\]/);
  assert.match(shell, /共通フロー/);
  assert.match(shell, /Gallery \/ Canvas \/ History \/ Jobs/);
  assert.match(shell, /社内βの状態/);
  assert.match(shell, /lightchainUnifiedFeatureCatalog\.filter/);
  assert.match(shell, /feature\.betaIncluded !== false/);
  assert.match(shell, /to="\/gallery"/);
  assert.match(shell, /to="\/canvas"/);
  assert.match(shell, /to="\/history"/);
  assert.match(shell, /to="\/jobs"/);
