  ...
  return (
    <section className={`glass-panel rounded-2xl p-5 ${className}`}>
      ...
      {topFailure ? (
        ...
        <Link to="/designProduction" className="mt-5 inline-flex items-center gap-2 rounded-lg bg-white/70 px-3 py-2 text-xs font-semibold text-neutral-700 transition hover:text-primary-700 dark:bg-white/10 dark:text-neutral-200">
          新しく生成する
          <ArrowRight className="h-3.5 w-3.5" />
        </Link>
      )}
