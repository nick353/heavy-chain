...
- Fast service tier is separate from reasoning effort and is inherited from the current user/thread selection. Adaptive role files do not pin `service_tier`, so one user's Fast choice is not imposed on another user or on another thread. The `fast_mode` feature flag exposes the switch but does not force it. OpenCode has no Codex Fast tier. Existing App threads retain their own tier until a supported per-thread setting operation is available.
