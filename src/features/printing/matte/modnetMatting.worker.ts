...
import { MODNET_ONNX_PRODUCTION_MODEL_URL } from './modnetMattingRuntimeContract';

const MODEL_URL = String(
  (import.meta.env ?? {}).VITE_MODNET_ONNX_MODEL_URL || MODNET_ONNX_PRODUCTION_MODEL_URL,
).trim();
const MODEL_EDGE = 256;
...
const createSession = async () => {
  ...
  try {
    ...
    console.warn('MODNet WebGPU session unavailable; using WASM.', webGpuError);
...
const resizeToNormalizedNchw = (rgba: Uint8ClampedArray, width: number, height: number) => {
  ...
  for (let y = 0; y < MODEL_EDGE; y += 1) {
    ...
    for (let x = 0; x < MODEL_EDGE; x += 1) {
      ...
      for (let channel = 0; channel < 3; channel += 1) {
        ...
        output[(channel * planeSize) + pixel] = (value - 0.5) / 0.5;
      }
    }
  }
  return output;
...
const readAlpha = (tensor: ort.Tensor, width: number, height: number) => {
  ...
  if (!(values instanceof Float32Array) || values.length < 1) throw new Error('modnet_alpha_output_invalid');
  ...
  for (let pixel = 0; pixel < source.length; pixel += 1) {
    source[pixel] = clamp(Number(values[pixel] ?? 0), 0, 1);
  }
...
self.addEventListener('message', (event: MessageEvent<MatteRequest>) => {
  ...
  void (async () => {
    if (!request.width || !request.height || !request.rgba) throw new Error('modnet_image_invalid');
    if (request.rgba.byteLength !== request.width * request.height * 4) throw new Error('modnet_image_shape_invalid');
    const session = await loadSession();
    const inputName = session.inputNames[0];
    if (!inputName) throw new Error('modnet_input_missing');
    const result = await session.run({
      [inputName]: new ort.Tensor(
        'float32',
        resizeToNormalizedNchw(new Uint8ClampedArray(request.rgba), request.width, request.height),
        [1, 3, MODEL_EDGE, MODEL_EDGE],
      ),
    });
    const output = result[session.outputNames[0]];
    if (!output) throw new Error('modnet_output_missing');
