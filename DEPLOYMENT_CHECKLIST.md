...
- [ ] deploy後のfrontend bundleにrevision固定Hugging Face URLが含まれ、旧`/models/u2net_cloth_seg.onnx`を参照しないことを確認する。browserから固定URLへのHEAD/GETが200かつCORS `*`であることもreadbackする
- [ ] 同一origin fallbackを選ぶ場合だけ`npm run build:rembg-cloth-model-same-origin`を実行し、dist modelの176,194,565 bytesと固定SHAを確認する
