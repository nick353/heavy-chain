-- Heavy Chain billing is not active yet, so generation must not be stopped by
-- the old monthly quota. Keep existence checks, free subscription bootstrap,
-- idempotency, stale reservation release, short-window rate limits, and usage
