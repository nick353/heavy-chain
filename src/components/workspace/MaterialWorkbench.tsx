...
import { CheckCircle2, FolderOpen, ImagePlus, Layers3, ScanLine, Scissors, SlidersHorizontal, Upload } from 'lucide-react';
...
export function MaterialWorkbench({
  ...
  const handleGallerySelect = (imageUrl: string, imageId: string, storagePath?: string) => {
    resetExtraction({
      imageUrl,
      fileName: `Gallery素材-${imageId}`,
    ...
    toast.success('既存Gallery素材を読み込みました');
  };
      ...
      <div className="mt-4 grid min-w-0 gap-4 2xl:grid-cols-[minmax(280px,1fr)_minmax(300px,0.86fr)]">
        <div className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-[#050707]">
          {enableGallerySelection && (
            <div className="flex items-center justify-between gap-3 border-b border-white/10 bg-neutral-950/80 px-3 py-2">
              <p className="text-[11px] font-semibold text-neutral-400">既存素材を優先</p>
              <button
                type="button"
                data-testid="material-gallery-select"
                aria-label="既存Gallery素材を選択"
                onClick={() => setShowGalleryModal(true)}
                className="inline-flex items-center gap-1.5 rounded-lg border border-cyan-300/30 bg-cyan-300/10 px-2.5 py-1.5 text-xs font-semibold text-cyan-100 transition hover:border-cyan-200/60 hover:bg-cyan-300/20"
              >
