...
AI フィッティングとマーケティングの parity MVP は実装済みです。さらに、両方の出力は Gallery/History に読み戻せるようになり、Canvas へは各ワークスペースから handoff できます。マーケティングと Fitting はどちらも、ブラウザから直接 Storage/REST write せず、Edge Function/service-role 経由で `generated-images` Storage と `generation_jobs` / `generated_images` に保存する形までローカル検証済みです。`/studio` `/models` `/video` `/lab` は local artifact、Canvas、Gallery、History に接続済みで、2026-06-21 の `workspace-generation-seed-local-v1` ではモデル、ポーズ、背景、動画構成、評価軸などを構造化して保存し、その保存内容から Gallery の「この内容で生成」または History の「生成へ進む」で `/generate` へ戻れるようになりました。2026-06-22 の `generation-context-readback-local-v1` では、その `/generate` 遷移に `sourceWorkspace`、`workflowVersion`、`sourceLabel`、`sourceResumePath`、`sourceMode` を載せ、生成エディタ側で allowlist hydrate した再開バナーと元ワークスペースへ戻るリンクを表示できるようにしました。続く `generation-source-provenance-local-v1` では、その状態で生成ボタンを押した結果を source provenance 付き local artifact として保存し、Gallery detail と History timeline から元ワークスペースへ戻れるようにしました。親 Playwright proof は `output/playwright/generation-context-readback-local-20260622/manifest.json` と `output/playwright/generation-source-provenance-local-20260622/manifest.json` に保存済みで、source query hydrate、生成結果metadata、Gallery/History source link、Storage/REST mutation なしを確認しています。今回の `source-context-summary-local-v1` では、その保存済み metadata をユーザーが読める `生成条件` として Gallery detail と History timeline に表示します。`/models` は `Street LOOK 30s` と `regular / 30s / medium / medium`、`/patterns` は `Bandana Grid`、`Smoke chain motif`、`参照素材: なし` まで確認済みです。さらに `model-matrix` Edge Function 側も、deploy 後の remote readback で同じ表示ができるよう `sourceReadback` と `modelCandidateLabel` を `generation_jobs.input_params` と `generated_images.metadata` に残す実装へ更新しました。親 proof は `output/playwright/source-context-summary-local-20260622/manifest.json` です。`/patterns` は `pattern-structured-context-local-v1` で、Bandana Grid などの `selectedPatternPreview` と、モチーフ、リピート、対象アイテム、パレット、ベクター化方針、参照素材を `/generate` の `design-gacha` request body と生成後 local artifact の provenance まで保持できるようになりました。参照素材が空欄でも `patternContext` は落ちません。親 Playwright proof は `output/playwright/pattern-structured-context-local-20260622/manifest.json` に保存しました。`/models` は `model-library-local-v1` で、Clean EC 20s、Street LOOK 30s、Premium AD 40s のモデル候補カード、フォーム同期、決定的 SVG preview、`selectedModelCandidate` metadata、Canvas image parameters 保存、Gallery/History から `model-matrix` への再利用、生成ボタン後の `標準 × 30代` 結果まで確認済みです。Street LOOK 30s は `modelMatrixBodyTypes=['regular']`、`modelMatrixAgeGroups=['30s']`、`modelMatrixSkinTone='medium'`、`modelMatrixHairStyle='medium'` として href と request body に反映されます。親 Playwright proof は `output/playwright/model-library-local-20260622/manifest.json` に保存しました。`/lab` は `lab-evaluation-local-v1` で、Material Lighting、Retail Readiness、Campaign Transfer の評価候補カード、フォーム同期、決定的スコア、SVG preview、`selectedLabExperiment` metadata、Canvas image parameters 保存、Gallery/History から `campaign-image` への再利用まで確認済みです。親 Playwright proof は `output/playwright/lab-evaluation-local-20260622/manifest.json` に保存しました。`/video` は `video-storyboard-local-v1` で、Launch Reel、Texture Close-up、Fit Check CTA の storyboard 候補選択、フォーム同期、決定的 SVG preview、`selectedVideoStoryboard` metadata、Canvas image parameters 保存、Gallery/History から `campaign-image` への再利用まで確認済みです。親 Playwright proof は `output/playwright/video-storyboard-local-20260622/manifest.json` に保存しました。`/patterns` は 2026-06-22 の `pattern-preview-local-v1` で、生成前 brief だけでなく、Emblem Lockup、Bandana Grid、Vector Path Caps の決定的 SVG プレビューを表示し、選択した候補を Canvas/Gallery/History の画像と metadata に残せるようになりました。加えて `workflow-query-prefill-local-v1` と 2026-06-22 の `workflow-board-local-v1` で、Dashboard の QuickWorkflows から EC商品画像、SNSキャンペーン、デザイン探索、グローバル展開のローカルワークフローボードへ入り、進捗、業務手順、成果物候補、関連ワークスペース、Canvas 導線を確認してから、業務別の初期値付き生成エディタへ進めるようになりました。次は明示承認後の deploy と production readback を行い、local handoff を実生成/readback へ接続するのが最短です。
...
同日の次スライスで、この client-side best-effort path は `marketing-workspace-artifact` Edge Function 経由へ移行しました。ブラウザは `supabase.functions.invoke('marketing-workspace-artifact')` だけを呼び、JWT 認証、brand editor 権限確認、service-role Storage upload、`generation_jobs` / `generated_images` insert、partial failure cleanup は Function 側に集約します。deploy と RLS migration はこのスライスでは行わず、E2E mock は Function success/failure、local fallback、Gallery/History duplicate 回避、client direct Storage/REST write が発生しないことを検証対象にします。
...
Gallery と History は、Supabase の `generated_images` / `generation_jobs` に加えてローカル成果物をマージして表示します。これにより、DB/RLS/deploy を新しく追加しない範囲でも、ユーザーは fitting/marketing/studio/patterns/video/lab の出力を検索し、詳細を開いて確認できます。Canvas handoff は、Marketing 画面の「キャンバスへ渡す」、Fitting 画面の履歴「編集」、Studio/Patterns/Video/Lab の「保存してCanvasへ」から行います。
...
Heavy Chain には、認証、ブランド設定、クレジット、ジョブ、履歴、ギャラリー、キャンバス、生成系 Edge Functions、Supabase storage/DB まわりの release 証跡があります。production app は 2026-06-21 時点の release doctor no STOP と production QA 証跡を持つため、次の Goal は新規デプロイや DB 変更ではなく、既存ワークスペース体験を機能接続する単位で切るべきです。

## 未実装差分

1. AI フィッティングは、parity MVP としてファイル選択、画像 data URL の API 引き渡し、生成 API 接続、結果プレビュー、ローカル履歴、失敗時の再試行、Gallery/History 保存、Canvas 編集 handoff まで実装済みです。さらに `model-matrix` Edge Function 経由でサーバー側 `generation_jobs`、Storage、`generated_images` への永続化形までローカル検証済みです。deploy 後の production readback、複数端末/再ログイン後の実 DB readback、削除/再生成の server state 接続は後続差分です。

2. マーケティングは、parity MVP として商品画像アップロード、ローカルジョブ、停滞/失敗/再試行、キャンバスプロジェクト生成、Gallery/History 保存まで実装済みです。best-effort remote save は Edge Function/service-role 経路へ移行済みですが、deploy と production readback は未実施です。サーバー側進捗 readback、AI アシスタントの実処理、永続プロジェクト保存、クレジット readback は後続差分です。
...
推奨順は、次に Marketing と Fitting の Edge Function persistence を deploy し、production readback で実 DB/Storage/job provenance を確認することです。Lightchain で実際に停滞を観察した領域なので、進捗、失敗復帰、途中編集、履歴復帰をサーバー状態に接続する価値があります。

1. 明示承認後に `marketing-workspace-artifact` と更新済み `model-matrix` を deploy し、production 環境で Storage object、`generation_jobs`、`generated_images`、Gallery/History readback、Canvas handoff を確認する。

2. `/marketing` と `/fitting` の progress、timeout、retry、failed state、credit readback、削除、再生成を DB/Edge Function の実状態に接続する。

3. `/models` `/studio` `/patterns` `/video` `/lab` は、今の local artifact/Canvas handoff と各ワークスペースの選択プレビューを実生成の入口にする。まずは既存アセットと履歴に接続し、実際の柄生成/ベクター変換、動画生成、店舗/展示シミュレーションは後続 Goal に分ける。

## Goal 用の完了条件

現在の Goal の次スライスは「Edge Function persistence の deploy と production readback」です。完了条件は、認証済みユーザーが `/marketing` と `/fitting` で作った出力を Supabase DB/Storage の履歴/ギャラリー/キャンバスで読み戻せ、再ログインまたは別ブラウザ相当でも確認でき、進捗、成功、停滞/失敗、再試行、編集画面への handoff を Playwright で証跡化できることです。

停止条件は、外部 API キー、DB/RLS、支払い、production deploy、クレジット課金仕様の変更が必要になった時点です。その場合は実装を止め、必要な変更と proof path を明示して human approval を待ちます。
