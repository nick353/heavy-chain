...
| H602 | billing/external publish | Billing quota enforcement/tester-exemption and purchase-proof groundwork is implemented locally, but the latest production readback is unsafe/incomplete: `generationQuotaEnforced=false`, `productionCheckoutEnabled=true`, `verifiedNoRealChargeProofCount=0`, and `transactionOrEntitlementReadback=false`. Apple readback confirms Developer Program agreement accepted, Paid Apps Agreement active for `2026-06-30 - 2026-10-19`, app version `1.0` is `配信準備完了`, monthly/yearly subscriptions approved at Japan prices `￥980.00` and `￥4,400.00`, and one Japan sandbox tester exists. The purchase-proof hardening, SHA-256, and metadata-allowlist migrations are source/readback artifacts, but they do not override the current production settings. Still human/operator gated: restore quota enforcement, disable checkout, obtain transaction/entitlement or verified no-real-charge proof, decide tax/refund/support scope, and make the final checkout/public-release decision. Current proof: `output/playwright/h602-production-billing-readback-20260630/summary.json` and `output/playwright/h602-billing-readiness-latest.json`. | open |
...
2. H602 billing/external publish:
   ...
   - Current production billing readback is fail-closed: `generationQuotaEnforced=false`, `productionCheckoutEnabled=true`, `verifiedNoRealChargeProofCount=0`, and `transactionOrEntitlementReadback=false`. The older statement that quota enforcement was enabled and checkout disabled is historical and must not be used as current proof. Proof: `output/playwright/h602-production-billing-readback-20260630/summary.json` and `output/playwright/h602-billing-readiness-latest.json`.
