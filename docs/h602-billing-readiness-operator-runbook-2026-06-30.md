...
- The historical migration/readback artifacts cover `20260630102537_enable_billing_enforcement_test_exemptions.sql` and the purchase-proof hardening layer, but the latest current production readback must be used for acceptance. It reports `generationQuotaEnforced=false`, `productionCheckoutEnabled=true`, `verifiedNoRealChargeProofCount=0`, and `transactionOrEntitlementReadback=false`; therefore H602 remains fail-closed. Proof: `output/playwright/h602-production-billing-readback-20260630/summary.json` and `output/playwright/h602-billing-readiness-latest.json`.
