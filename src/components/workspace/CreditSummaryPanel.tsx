...
export function CreditSummaryPanel({ summary, className = '' }: CreditSummaryPanelProps) {
  const usedPercent = summary.monthlyQuota > 0
    ? Math.min(100, Math.round(((summary.usedUnits + summary.reservedUnits) / summary.monthlyQuota) * 100))
    : 0;
    ...
    <section className={`glass-panel rounded-2xl p-5 ${className}`}>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-xl font-semibold text-neutral-950 dark:text-white">クレジット</h2>
        </div>
      ...
      <div className="mt-5">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400">残り / {summary.planName}</p>
          </div>
        ...
        <div className="mt-4 h-2 rounded-full bg-neutral-100 dark:bg-surface-800">
          <div className="h-full rounded-full bg-gradient-to-r from-primary-500 to-gold-DEFAULT" style={{ width: `${usedPercent}%` }} />
        </div>
