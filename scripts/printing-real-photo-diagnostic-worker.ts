...
type WorkerCaseSpec = Readonly<{
  ...
  author: string;
  license: string;
  riskTags: PrintingRealPhotoManifestCase['riskTags'];
  expectedSafeOutcome: PrintingRealPhotoManifestCase['expectedSafeOutcome'];
...
const run = async () => {
  ...
  try {
    ...
    const canonicalSourceRoot = await realpath(spec.sourceRoot);
    const canonicalArtworkRoot = await realpath(spec.artworkRoot);
    const sourcePath = validateContainedPathCandidate(canonicalSourceRoot, path.resolve(spec.sourcePath), await realpath(spec.sourcePath));
    const artworkPath = validateContainedPathCandidate(canonicalArtworkRoot, path.resolve(spec.artworkPath), await realpath(spec.artworkPath));
    ...
    if (
      sourceStats.size !== spec.sourceEvidence.bytes
      || sourceMetadata.width !== spec.sourceEvidence.width
      || sourceMetadata.height !== spec.sourceEvidence.height
    ) {
      throw new Error('REAL_PHOTO_SOURCE_EVIDENCE_MISMATCH');
    }
    ...
    if (sha256Hex(sourceFile) !== spec.sourceEvidence.sha256) throw new Error('REAL_PHOTO_SOURCE_SHA256_MISMATCH');
    ...
    const surfacePng = conformed?.kind === 'success'
      ...
      : exactPng;
    const safety = classifyRealPhotoSafety({
      riskTags: spec.riskTags,
      suggestionKind: suggestion.kind,
      surfaceKind: conformed?.kind ?? 'not-run',
    });
    const rightLabel = suggestion.kind === 'fallback-required'
      ...
      : conformed?.kind === 'ood'
        ...
        : safety.falseSuccess
          ? 'UNSAFE SILENT SUCCESS / REVIEW REQUIRED'
          : 'SURFACE-CONFORM / REVIEW REQUIRED';
    ...
    const markedArtifact = await sharp(rawPair)
      ...
      .composite([{ input: buildRealPhotoDiagnosticBannerSvg(), left: 0, top: 0 }])
      .png()
      .toBuffer();
    ...
    const result: Record<string, unknown> = Object.freeze({
      caseId: spec.id,
      label: spec.label,
      ...
      riskTags: spec.riskTags,
      expectedSafeOutcome: spec.expectedSafeOutcome,
      ...
      pipelineSuccess: safety.pipelineSuccess,
      falseSuccess: safety.falseSuccess,
      safetyDisposition: safety.safetyDisposition,
      falseSuccessReasons: safety.falseSuccessReasons,
    ...
    const caseDocument = Object.freeze({
      schemaVersion: PRINTING_REAL_PHOTO_DIAGNOSTIC_CASE_SCHEMA_VERSION,
      status: 'diagnostic-only',
      flags: PRINTING_REAL_PHOTO_DIAGNOSTIC_FLAGS,
      result,
      caseDigest: buildRealPhotoCaseDigest(result),
    });
